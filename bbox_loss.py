...
class MultiboxLoss(nn.Module):
    ...
    def forward(self, confidence, pred_loc, gt_class_labels, gt_bbox_loc):
        ...
        with torch.no_grad():
            ...
            num_pos = pos_flag.sum(dim=1, keepdim=True).long().float().sum()

        # Loss for the classification
        num_classes = confidence.shape[2]
        sel_conf = confidence[sel_flag].clone()
        conf_loss = F.cross_entropy(sel_conf.reshape(-1, num_classes), gt_class_labels[sel_flag].clone().long(), reduction='sum') / num_pos

        # Loss for the bounding box prediction
        # TODO: implementation on bounding box regression
        sel_pre_loc = pred_loc[pos_flag, :].reshape(-1, 4).clone()
        sel_gt_loc = gt_bbox_loc[pos_flag, :].reshape(-1, 4).clone()
        loc_huber_loss = F.smooth_l1_loss(sel_pre_loc, sel_gt_loc, reduction='sum') / num_pos
